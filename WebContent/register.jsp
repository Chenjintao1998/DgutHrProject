...
<body>
	<form id="ff" method="post">
		<div class="easyui-panel"
			...
			<div style="margin-bottom: 20px">
				<input id="phone" label="手机号码:" class="easyui-numberbox" name="phone"
					   data-options="required:true,validType:['phoneNum','phonewww']"
					   style="width: 50%; height: 34px; padding: 10px;">
			</div>
			<div style="margin-bottom: 20px">
				<input id="code" label="验证码:" class="easyui-numberbox"
					   style="width: 30%; height: 34px; padding: 10px;">
				<!-- <button type="button"  class="easyui-linkbutton c1" id="cod" name="cod"
						style="width: 20%; height: 34px; padding: 10px;" onclick="">发送验证码</button> -->
						<input id="cod" name="cod" class="easyui-linkbutton c1" style="width: 20%; height: 34px; padding: 10px;" type="button" value="获取验证码" onclick="checkPhoneCode()">
			</div>
			<div style="margin-bottom: 20px">
			...
			<div style="margin-top: 20px; text-align: center;">
				<a href="javascript:void(0)" class="easyui-linkbutton c1"
					style="width: 120px" onclick="submitForm()">注册</a>
			</div>
		</div>
	</form>
	<script type="text/javascript">
		...
		function checkPhoneCode() {
			...
			$.post("sendCode", json, function(data) {
				if(data == "-1") {$.messager.alert('错误提示', '验证码发送失败!');}
				else{code = data;}
			});
			// $.post("testCode",json,function (data) {
			// 	code = data;
			// })
		}
		$
		...
		function submitForm() {
			 $('#ff').form('submit', {
				...
				onSubmit : function() {
					...
					if(code == 0 || code != $('#code').val()){
						$.messager.alert('错误提示', '验证码错误', 'error');
						return false;
					}
					return $(this).form('enableValidation').form('validate');
				},
